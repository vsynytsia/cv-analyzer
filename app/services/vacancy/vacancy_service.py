...
async def fetch_filtered_vacancies(
    http_client: http.AsyncHTTPClient, filters: Iterable[JobSearchFilter], limit: int | None = None
) -> list[Vacancy]:
    urls = await concurrency.gather(
        items=filters,
        coro_factory=lambda filter_: fetch_vacancies_urls_with_filter(http_client, filter_, limit),
...
async def fetch_vacancies_urls_with_filter(
    http_client: http.AsyncHTTPClient, filter_: JobSearchFilter, limit: int | None = None
) -> list[str]:
    ...
    return extractor(response.content, limit)


def get_vacancy_urls_extractor(filter_: JobSearchFilter) -> Callable[[bytes, int | None], list[str]]:

...
from app.api.deps import GeminiClientDep, HttpxClientDep, JinjaEnvDep
from app.core.exceptions import GeminiAPIError, TranslationAPIError, UnsupportedLanguageError
from app.schemas.vacancy import ScoredVacancy
from app.services import cv_service, file_service, score_service, vacancy_service
...
async def process_cv(
    cv: UploadFile, gemini_client: GeminiClientDep, httpx_client: HttpxClientDep, jinja_env: JinjaEnvDep
) -> list[ScoredVacancy]:
    logger.info(f"Started processing {cv.filename}")

    if not file_service.is_pdf(cv):
        raise HTTPException(
            status.HTTP_415_UNSUPPORTED_MEDIA_TYPE, "Unsupported file type. Currently, only PDFs are supported."
        )
    if file_service.file_size_exceeds_limit(cv):
    ...
    try:
        ...
        vacancy_search_filters = await cv_service.extract_search_filters(gemini_client, jinja_env, cv_str)
        logger.info("Extracted search filters from CV")

        vacancies = await vacancy_service.fetch_filtered_vacancies(httpx_client, vacancy_search_filters)
